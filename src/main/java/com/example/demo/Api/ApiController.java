...
public class ApiController {
    ...
    public List<User> NguoiDung(@RequestParam(name = "id", required = false) List<Long> id) {
    ...
    @ResponseBody
    public List<String> LoginUser(@RequestBody List<LoginDto> users) {
        List<String> results = new ArrayList<>();
        for (LoginDto user : users) {
            Optional<User> existed = userReponse.findByEmail(user.getName());
